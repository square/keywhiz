...
public class MembershipResourceTest {
  ...
  @Before public void setUp() {
    resource = new MembershipResource(aclJooqDao);
  }
  ...
  @Test public void canAllowAccess() {
    ...
    verify(aclJooqDao).findAndAllowAccess(66, 55);
  }

  @Test(expected = NotFoundException.class)
  public void missingSecretAllow() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndAllowAccess(3, group.getId());
    resource.allowAccess(user, new LongParam("3"), new LongParam(Long.toString(group.getId())));
  }

  @Test(expected = NotFoundException.class)
  public void missingGroupAllow() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndAllowAccess(secret.getId(), 98);
  ...
  @Test public void canDisallowAccess() {
    ...
    assertThat(response.getStatus()).isEqualTo(200);
    verify(aclJooqDao).findAndRevokeAccess(secret.getId(), group.getId());
  }

  @Test(expected = NotFoundException.class)
  public void missingSecretDisallow() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndRevokeAccess(2, group.getId());
    resource.disallowAccess(user, new LongParam("2"), new LongParam(Long.toString(group.getId())));
  }

  @Test(expected = NotFoundException.class)
  public void missingGroupDisallow() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndRevokeAccess(secret.getId(), 3543);
  ...
  public void canEnroll() {
    resource.enrollClient(user, new LongParam(Long.toString(client.getId())),
        new LongParam(Long.toString(group.getId())));
    verify(aclJooqDao).findAndEnrollClient(client.getId(), group.getId());
  }

  @Test(expected = NotFoundException.class)
  public void enrollThrowsWhenClientIdNotFound() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndEnrollClient(6092384, group.getId());
  ...
  public void enrollThrowsWhenGroupIdNotFound() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndEnrollClient(client.getId(), 0xbad);
  ...
  public void canEvict() {
    resource.evictClient(user, new LongParam(Long.toString(client.getId())),
        new LongParam(Long.toString(group.getId())));
    verify(aclJooqDao).findAndEvictClient(client.getId(), group.getId());
  }

  @Test(expected = NotFoundException.class)
  public void evictThrowsWhenClientIdNotFound() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndEvictClient(60984, group.getId());
  ...
  public void evictThrowsWhenGroupIdNotFound() {
    doThrow(IllegalStateException.class).when(aclJooqDao).findAndEvictClient(client.getId(), 0xbad2);
    resource.evictClient(user, new LongParam(Long.toString(client.getId())),
